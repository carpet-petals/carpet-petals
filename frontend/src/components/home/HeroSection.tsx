...
interface Props {
  data: HeroContent | null; // null = still loading
}
...
export default function HeroSection({ data }: Props) {
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      {/* Background — visible immediately, image fades in when loaded */}
      ...
      <div className="relative z-10 container-max px-4 sm:px-6 lg:px-8 text-center text-white">
        ...
        {/* Skeleton while loading, content when ready */}
        {!data ? (
          <div className="space-y-4 flex flex-col items-center">
            <div className="h-12 w-80 bg-white/10 animate-pulse rounded" />
            <div className="h-5 w-96 bg-white/10 animate-pulse rounded" />
            <div className="h-5 w-72 bg-white/10 animate-pulse rounded" />
          </div>
        ) : (
          <>
            {tagline && (
